...
export const expectedFailure = '验证构建缺少 beta 动作'

export function mutateVerificationProject(tempRoot) {
  replaceFileContent(join(tempRoot, 'app/main/actions/beta.ts'), 'beta.echo', 'beta.absent')
  replaceFileContent(join(tempRoot, 'app/preload/api/beta.ts'), 'beta.echo', 'beta.absent')
}
