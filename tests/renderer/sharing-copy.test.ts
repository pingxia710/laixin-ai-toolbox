...
it('页面含风险提示与租约、交付、使用约定、取消说明文案', async () => {
  const api = stubSharingApi()
  const { root, dispose } = await mountView(api)
  const text = root.text()
  expect(text).toContain('账号租用和 API 租用')
  expect(text).toContain('需求者和供给者都按标准模板发布')
  expect(text).toContain('共享账号可能违反部分厂商的服务条款')
  root.button('账号租用')!.click()
  await flush()
  root.button('查看租约与办理说明')!.click()
  await flush()
